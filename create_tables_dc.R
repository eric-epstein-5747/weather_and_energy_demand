...
## Create a table containing electricity demand data for Washington, DC----
...
# Create year and month columns (to join on later)
...
# Let's create some functions to switch between numerical & character representations of months:
...
## Create a table containing weather data for Washington, DC----
...
# Convert numerical months to character representations, so we can join to sales data:
f <- function (yr) {
...
# Join the weather data to the energy data so we can start looking for patterns----
sales_weather <- left_join(retail_sales, weather_means, by=c("year","month"))
sales_weather$nmonth <-  g(sales_weather$month)

# Export the joined data to the project directory so we don't have to recreate it for each session.
